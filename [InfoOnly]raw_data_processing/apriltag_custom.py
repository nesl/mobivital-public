...
def main():
    ...
    if os.path.splitext(options.input)[1] != ".bag":
        print("The given file is not of correct file format.")
        print("Only .bag files are accepted")
        exit()
    print("Prepareation Finished.")
    try:
        # Create pipeline
        ...
        rs.config.enable_device_from_file(config, options.input)
    ...
    #    config.enable_stream(rs.stream.depth, 1080, 720, rs.format.z16, 30)
    #    config.enable_stream(rs.stream.color, 1080, 720, rs.format.bgr8, 30)
        ...
        cv2.namedWindow("Aligned Image", cv2.WINDOW_AUTOSIZE)
        ...
        fp = open(os.path.splitext(options.input)[0] + ".csv", 'w')
        ...
        while True:
            ...
            if time.time() - start_time > 1:
                if (realtime_flag):
                    ...
                    playback.resume()
                # playback.pause()
                past_frame_time = frame_time
                frame_time = frames.get_timestamp()/1000.0
                # print(frame_time)
                if frame_time == past_frame_time:
                    continue
                if frame_time - past_frame_time < -1:
                    print(frame_time)
                    print(past_frame_time)
                    break
                aligned_frames = align.process(frames)

                # use only when we need to print camera intrinsics after alignment
                # if not flag:
                #     prof = aligned_frames.get_profile().as_video_stream_profile()
                ...
                if len(color_image.shape) == 3:
                    overlay = color_image // 2 + dimg[:, :, None] // 2
                else:
                ...
                active_tag_detected = 0
                for i, detection in enumerate(detections):
                    # print( 'Detection {} of {}:'.format(i+1, num_detections))
                    # print()
                    # print(detection.tostring(indent=2))
                    if detection.tag_id == active_tag:
                        active_tag_detected = 1
                        if options.camera_params is not None:

                            pose, e0, e1 = det.detection_pose(detection,
                                                            options.camera_params,
                                                            options.tag_size)
                            writer.writerow(np.insert(pose.flatten(),0,frame_time))
                            # import pdb; pdb.set_trace()
                ...
                if not active_tag_detected:
                    print("Tag {} not detected at timestamp {}".format(active_tag, frame_time))
                ...
                # images = np.hstack((depth_color_image, overlay))
